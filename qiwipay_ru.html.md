...
merchant_cheque|Опционально|string(4096)|Данные чека в BASE64 [Данные для кассового чека по 54-ФЗ](#cheque)
...
Безопасная сделка от QIWI обеспечивает прозрачный и надежный механизм оплаты работ без риска потери денег как для заказчика, так и для исполнителя. Для того, чтобы подключить Безопасную сделку, необходимо обратиться к вашему курирующему менеджеру.
...
* авторизация д/с через вызов [API](#sale) или [WPF](#redirect) (код операции `1`)
...
* авторизация д/с через вызов [API](#sale) или [WPF](#redirect) (код операции `1`, `3`)
* [подтверждение операции](#capture) (код операции `5`)
...
<aside class="success">
Для активации сервиса приема фискальных данных необходимо сообщить менеджеру ИНН, с которым организация зарегистрирована в сервисе по аренде онлайн касс.
...
Для каждого нового ТСП в системе создаются `merchant_site` и по умолчанию включаются в режим тестирования. Также можно запросить переключение в режим тестирования любого своего `merchant_site`, либо добавление нового `merchant_site` в тестовом режиме через своего менеджера.

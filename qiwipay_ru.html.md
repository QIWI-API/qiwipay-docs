...
<aside class="notice">
Алгоритм создания подписи совпадает с остальными запросами. Однако в данном случае участвуют не все поля, а только отмеченные <b>+</b> в столбце <i>Участвует в sign</i> в таблице выше.
</aside>
...
* `apple_pay_encoded_payment_token` - полученные от Apple платежные данные (`paymentData` из [объекта ApplePayPaymentToken](https://developer.apple.com/library/archive/documentation/PassKit/Reference/PaymentTokenJSON/PaymentTokenJSON.html))
...
Для тестирования операций оплаты используются [боевые url](#urls) сервисов оплаты.

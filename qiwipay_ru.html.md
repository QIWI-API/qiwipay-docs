...
Подсумма выплат отвечает за денежные средства, которые будут выплачены исполнителю при завершении безопасной сделки.

Подсумма комиссий отвечает за комиссии Площадки, на которой выполняется сделка, и QIWI. Из данной подсуммы QIWI удержит комиссию за проведение операции, а остаток перечислит Площадке.

Подсуммы передаются в [авторизационном запросе](#sale) или в [вызове WPF](#redirect) в поле `cf3` через точку с запятой: 

`"cf3": "подсумма_выплаты;подсумма_комиссий"`

<aside class="success">Сумма авторизации (<code>amount</code>) должна равняться сумме ее подсумм. Пример:<br><code>"amount": "500.00"</code><br><code>"cf3": "300.00; 200.00"</code>
</aside>

## Безопасная сделка с выплатой на кошелек

Данный вид безопасной сделки состоит из двух этапов:

* авторизация списания средств через вызов [API](#sale) или [WPF](#redirect) (код операции `1`)
...
Транзакцию можно отменить полностью или частично только до подтверждения операции. Чтобы совершить отмену, выполните [запрос отмены](#cancel) (код операции `6`) с  дополнительным полем `cf3` с отменяемыми подсуммами (в том же формате, что и при авторизации). 
...
Данный вид безопасной сделки состоит из трех этапов:
...
Для отмены или возврата выполните запрос [отмены](#cancel) (код операции `6`) или [возврата](#refund) (код операции `7`) и в дополнительном поле `cf3` укажите отменяемые подсуммы (в том же формате, что и при авторизации). 
...
Вам понадобится самостоятельно произвести интеграцию с Apple. Данная интеграция позволит верифицировать сайт ТСП и получать платежные данные пользователя. Требования к ТСП для интеграции Apple Pay на вашей веб-странице:
...
При процессинге платежей ApplePay на текущий момент при выполнении операции покупки поддерживаются [следующие коды операций](#sale_types) (`opcode`): `sale` (1) и `auth` (3).

Для отправки платежных данных в QIWI необходимо в [запросе авторизации](#sale) передать параметр `apple_pay_encoded_payment_token`, содержащий зашифрованные платежные данные от Apple.
 
Значение поля `apple_pay_encoded_payment_token` представляет из себя закодированную строку. Эта строка (как и все остальные значения полей в JSON-теле запроса) должна участвовать в вычислении [подписи запроса](#sign) (`sign`).
...
Здесь `json` – это JSON-объект со следующей структурой:
...
В этом объекте должны находиться все поля, получаемые от Apple в соответствии со структурой "Top-Level Structure". См. [документацию](https://developer.apple.com/library/archive/documentation/PassKit/Reference/PaymentTokenJSON/PaymentTokenJSON.html#//apple_ref/doc/uid/TP40014929-CH8-SW2)

**Шаг 1 (`to_bytes`)**: Объект `json` конвертируется в массив байтов в соответствии с кодировкой UTF-8.

**Шаг 2** (`compress_bytes_with_deflate_alg`): Байты, полученные на предыдущем шаге сжимаются с помощью [алгоритма DEFLATE](https://ru.wikipedia.org/wiki/Deflate). При выполнении компрессии с помощью алгоритма DEFLATE необходимо следовать спецификации RFC1950 и обеспечить запись 'zlib header' в начало потока со сжатыми данными.
...
В результате выполнения всех шагов мы получаем строку с закодированным значением, которое и подставляется в значение поля `apple_pay_encoded_payment_token`.
...
Для каждого нового ТСП в системе создаются `merchant_site` и по умолчанию включаются в режим тестирования. Также можно запросить переключение в режим тестирования любого своего `merchant_site`, либо добавление нового `merchant_site` в тестовом режиме через вашего сопровождающего менеджера.
